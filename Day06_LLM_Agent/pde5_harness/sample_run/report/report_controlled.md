...
것은 **6.9%** 로 통제 세트 전체의
28.2% 보다도 낮다. **이 연구에서 가장 강한 상관이 자세가
...
|---|---|---|---|---|---|
| far | 55 | 10.6 | 16 | — | **해석 불가** |
| mid | 51 | 19.9 | 0 | 5.74 | 31.4% |
| near | 52 | 26.9 | 0 | 3.27 | 25.0% |
| **해석 가능 대역 합** | **103** | **23.5** | **0** | **3.79** | **28.2%** |
| (참고) far 포함 전체 | 158 | 19.0 | 16 | 4.50 | 19.0% |
...
그 통합값도 오염된다. 초안은 far 를 포함한 19.0% 를 헤드라인으로
썼는데, 그것을 28.2% 로 정정한다. 참고를 위해 두 값을 모두 표에 남겼다.

**해석 가능한 대역에서도 자세의 72% 가 C2 대조를 실패시킨 것과
같은 기준을 넘지 못한다** (n=103). 즉 §3.4 의 "신호 없음"은 **대부분 결정 자세 틀 밖에
놓인 자세에 매긴 점수**에 대한 진술이고, near 대역의 유의한 상관도
25% 만 틀 안에 있는 자세들에서 나왔다. 이것이 본 연구의
...
72% 가 2 Å 기준을 넘지 못한다 (해석 가능 대역
103건 기준). C2 대조가 실패했다는
...
연구에서 그 비율은 28.2% 였고, 이 사실 없이 상관값만
...
8. **자세 타당도 보고** — 1위 자세의 72% 가 2 Å 기준
...
13. **far 대역 자세 타당도 값 삭제 + 헤드라인 정정** — 리뷰가 "해석할 수 없다고 방금
   ...
   10.6원자이고 55건 중
   16건이 하한 8원자에 걸려 있어 RMSD 가 자세 정확도를 재지
   못한다. 그 값을 표에서 비웠다. **그리고 같은 이유로 통합 헤드라인도 정정했다** —
   해석 불가한 대역을 통합값에 넣으면 통합값도 오염되므로,
   19.0% (far 포함) 대신
   28.2% (mid·near, n=103) 를 쓴다. 즉 "자세의
   81% 가 기준 밖" 이 아니라
   "72% 가 기준 밖" 이다. 해석 가능 여부는
