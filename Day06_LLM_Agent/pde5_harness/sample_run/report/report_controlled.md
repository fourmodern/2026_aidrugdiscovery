...
**결론.** **이 집합에서는 전체 순위 신호 자체가 없다** (ρ = -0.079, 95% CI -0.230–+0.076, 순열 p = 0.31878). 앞선 n=30 설계에서 얻은 상관은 재현되지 않았다. **다만 그 차이의 원인이 골격 교란이라는 설명은 검정 결과 기각되었다** (§3.5) — 원인은 특정하지 못한다. 다만 **결정 구조와 같은 계열 안에서는 신호가 남는다** (near 대역 ρ = -0.397, p = 0.00365, AUC 0.792) — 채점 함수는 구조가 유래한 화학형 안에서만 작동한다. 정량 예측은 사전 기준(Q² ≥ 0.3)을
충족하지 못했다.
...
**0.8%** 에 불과하고, 옛 데이터의
점수-Tanimoto 상관은 -0.125
(p=0.50747)로 관계 자체가 없다. 옛 설계를 새 데이터에
...
존재)을 **전수** 받아 2538개 레코드를 얻었다. 화합물당 pChEMBL 중앙값 (반복 측정 병합) —
...
늘리면 값이 좋아진다 — 따라서 이 표의 C1 은 §3.2 상단 대조(9개 모드)와
...
| 검정 | 결과 | 해석 |
|---|---|---|
| ① 옛 데이터에서 Tanimoto 통제 | -0.538 → -0.534 (감쇠 0.8%) | 골격 교란이 아니다 |
| ② 옛 데이터의 점수 vs Tanimoto | -0.125 (p=0.50747) | 점수가 골격을 따라가지 않았다 |
...
제외했다 (전체 163건 중 5건). 이 지표는 대칭 보정이 없는 단일 매핑
...
0.619 로 서로 다른데도 핵심 잔기는 양쪽 모두 전 화합물에서 접촉한다.
해석은 §4.3 에서 다룬다.
...
**이 집합에서는 전체 순위 신호 자체가 없다** (ρ = -0.079, 95% CI -0.230–+0.076, 순열 p = 0.31878). 앞선 n=30 설계에서 얻은 상관은 재현되지 않았다. **다만 그 차이의 원인이 골격 교란이라는 설명은 검정 결과 기각되었다** (§3.5) — 원인은 특정하지 못한다. 다만 **결정 구조와 같은 계열 안에서는 신호가 남는다** (near 대역 ρ = -0.397, p = 0.00365, AUC 0.792) — 채점 함수는 구조가 유래한 화학형 안에서만 작동한다.
...
나쁘다.** 대역별 평균 점수도 far -10.20 · mid -9.63 · near -9.99
kcal/mol 로 가장 안 닮은 대역이 가장 좋은 점수를 받는다. 점수 상위 16건의 대역 구성은
far 11 · mid 1 · near 4 로,
무작위 기대(각 약 5.3건) 대비 far 가 뚜렷이 과대표집된다.
...
9. **검정력이 부족하다.** 80% 검정력에서 검출 가능한 최소 |ρ| 은 전체 0.218 · far 0.355 · mid 0.383 · near 0.38 · near AUC 0.478 다. far·mid 대역의
...
**선별도 전체적으로는 작동하지 않는다.** **이 집합에서는 전체 순위 신호 자체가 없다** (ρ = -0.079, 95% CI -0.230–+0.076, 순열 p = 0.31878). 앞선 n=30 설계에서 얻은 상관은 재현되지 않았다. **다만 그 차이의 원인이 골격 교란이라는 설명은 검정 결과 기각되었다** (§3.5) — 원인은 특정하지 못한다. 다만 **결정 구조와 같은 계열 안에서는 신호가 남는다** (near 대역 ρ = -0.397, p = 0.00365, AUC 0.792) — 채점 함수는 구조가 유래한 화학형 안에서만 작동한다.
