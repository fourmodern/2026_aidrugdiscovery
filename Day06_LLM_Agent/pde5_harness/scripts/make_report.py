#!/usr/bin/env python3
"""IMRAD 보고서 생성 — 결과값은 전부 원자료에서 주입한다.

[개정 2026-09-04, 리뷰 1차 반영]
  이전 판은 (a) 게이트 4개가 모두 차단한다고 적었으나 selectivity 게이트는 반환값이
  버려져 차단하지 않았고, (b) 훅을 "기계가 강제"라 적었으나 훅은 비차단 리마인더이며,
  (c) "Lipinski 위반이 드물다"고 적었으나 실제로는 10건 중 7건이 MW 500 을 넘었다.
  세 진술 모두 사실 오류였다. 이 판은 그 오류를 정정하고, 발견 자체를 결과로 보고한다.

주의: 이 스크립트는 *결과 수치*를 주입한다. 서술문·절 제목·환경 문자열은 사람이 쓴다.
"""
import argparse, json, re, hashlib
from datetime import datetime, timezone
from pathlib import Path

QED_MIN, SA_MAX = 0.5, 6.0


def load_objs(path: Path):
    txt = path.read_text(); dec = json.JSONDecoder(); i = 0; objs = []
    while True:
        m = re.search(r"[\[{]", txt[i:])
        if not m: break
        st = i + m.start()
        try:
            o, e = dec.raw_decode(txt[st:]); objs.append(o); i = st + e
        except ValueError:
            i = st + 1
    return objs
...
def main():
    ...
    ap.add_argument("--gatelog", default="sample_run/run_gatelog.txt")
    ap.add_argument("--out", default="outputs/report_pde5.md")
    a = ap.parse_args()
    root = Path(a.run).resolve().parent.parent
    src, gl = Path(a.run), Path(a.gatelog)
    objs = load_objs(src)
    tgt = next(o for o in objs if isinstance(o, dict) and "accession" in o)
    P = next(o for o in objs if isinstance(o, list) and o and "QED" in o[0])

    env_dir = src.parent / "envelopes"
    ch_env = json.loads((env_dir / "chembl-actives.json").read_text())
    tg_env = json.loads((env_dir / "target-lookup.json").read_text())
    sel_env = json.loads((env_dir / "selectivity-check.json").read_text())
    ids = [r.get("molecule_chembl_id", "?") for r in ch_env["result"]]
    smiles = [r.get("canonical_smiles", "") for r in ch_env["result"]]

    n = len(P); npass = sum(r["gate_pass"] for r in P); nfail = n - npass
    nro5 = sum(r["Ro5_pass"] for r in P)
    n_mw_over = sum(1 for r in P if r["MW"] > 500)
    n_zero = sum(1 for r in P if r["MW"] <= 500 and r["logP"] <= 5
                 and r["HBD"] <= 5 and r["HBA"] <= 10)
    sa_max = max(r["SA"] for r in P)
    qeds = sorted(r["QED"] for r in P)
    top_fail = max((r for r in P if not r["gate_pass"]), key=lambda r: r["QED"])
    low_pass = min((r for r in P if r["gate_pass"]), key=lambda r: r["QED"])
    sweep = [(t, sum(r["QED"] >= t for r in P)) for t in
             (0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70)]
    sw = dict(sweep)
    n_skills = len([d for d in (root / ".claude" / "skills").iterdir() if d.is_dir()])
    log = gl.read_text().strip().splitlines()
    negctl = (src.parent / "negative_control_gatelog.txt")
    neg = negctl.read_text().strip() if negctl.exists() else "(미실행)"
    refs = (src.parent / "references.md").read_text().strip()
    envp = src.parent / "environment.json"
    envd = json.loads(envp.read_text()) if envp.exists() else {}
    chp = src.parent / "chembl_status.json"
    chembl_ver = json.loads(chp.read_text()).get("chembl_db_version", "확인 실패") if chp.exists() else "확인 실패"
    envline = " · ".join(f"{k} {v}" for k, v in envd.items())
    ts = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d")

    rows = "\n".join(
        f"| {ids[P.index(r)] if r in P else '?'} | {r['MW']:.1f} | {r['logP']:.2f} | "
        f"{r['HBD']} | {r['HBA']} | {r['TPSA']:.1f} | {r['QED']:.3f} | {r['SA']:.2f} | "
        f"{'통과' if r['Ro5_pass'] else '위반'} | **{'PASS' if r['gate_pass'] else 'FAIL'}** |"
        for r in sorted(P, key=lambda x: -x["QED"]))
    sweep_row = " | ".join(str(c) for _, c in sweep)
    conf = sel_env["result"]["confirmation_required"]
    mech = sel_env["result"]["cross_inhibition_mechanism"]

    md = f"""# 검증 게이트를 갖춘 신약탐색 에이전트 하네스의 단일 실행 기록
## 그리고 그 게이트 중 둘이 반증 불가능했다는 발견

**작성일** {ts} · **버전** 2.0 (1차 리뷰 반영) · **원자료** `sample_run/run_stdout.json`
(SHA-256 앞 16자리 `{sha16(src)}`), `sample_run/run_gatelog.txt` (`{sha16(gl)}`)
...
({tgt['accession']}) 저해제 선별에 적용해 1회 실행했다 [6,5]. 물성은 RDKit 으로 계산했다 [4].

**결과.** 게이트 로그는 {len(log)}단계 전부 PASSED 였다. 그러나 로그만으로는 강제 여부를
...
화합물 {n}건 중 약물성 게이트를 {npass}건이 통과했다.
...
| `.claude/skills/*/SKILL.md` | 도구별 호출 시점·명령·검증 항목 ({n_skills}종) | 문서 — 강제력 없음 |
...
표적은 UniProt `{tgt['accession']}` 조회로 확인했다 [6]. 후보는 ChEMBL `CHEMBL1827` 의
활성 레코드 {n}건이다 [5]. **정렬을 지정하지 않은 조회 결과의 앞에서부터 {n}건을 취한
...
약물성 게이트는 `Ro5 위반 1개까지 허용 AND QED >= {QED_MIN} AND SA <= {SA_MAX}` 이다.
...
2회차는 결함을 고친 뒤다. 두 실행의 화합물 {n}건은 동일하다(SMILES·QED 전수 대조).
...
실행 환경은 {envline} 이며 `sample_run/environment.json` 에 기록했다.
데이터베이스는 **{chembl_ver}** 이다 (`sample_run/chembl_status.json`).
...
{len(log)}단계 전부 PASSED 다. 그러나 이 로그만으로는 강제 여부를 판정할 수 없다.
...
| mol-properties 게이트 | 차단함 | 동일. 단 약물성 판정(`gate_pass`)은 `verification.passed` 에 포함되지 않아 {n}건 전부 탈락해도 이 단계는 통과한다 |
...
UniProt 조회 결과는 accession `{tgt['accession']}`, 유전자 `{tgt['gene']}`,
단백질 `{tgt['protein']}`, 길이 {tgt['length']} 잔기다.
...
이 보고의 초판은 게이트 로그 {len(log)}줄을 근거로 "계약이 강제되었다"고 적었다.
...
실제로 쓴 {QED_MIN} 에서 좌우로 0.05 씩 옮기면 통과가 {sw[0.45]}건과 {sw[0.55]}건으로
갈린다. 0.35 까지 내리면 {sw[0.35]}건 전부가 통과해 게이트가 아무것도 걸러내지 못한다.
임계 근방에서는 QED {top_fail['QED']:.3f} 가 탈락하고 {low_pass['QED']:.3f} 가 통과했는데
차이는 {low_pass['QED'] - top_fail['QED']:.3f} 다. 산출은 측정값보다 임계 선택에 민감하다.

### 4.3 Lipinski 규칙은 왜 아무것도 걸러내지 못했는가

Ro5 는 {nro5}/{n} 을 통과시켜 한 건도 탈락시키지 않았다. 초판은 이를 "이미 최적화된 계열이라
위반이 드물기 때문"으로 해석했으나 **데이터가 그 해석을 반박한다.** 실제로는 {n_mw_over}/{n} 이
MW 500 을 초과하고, 위반이 하나도 없는 화합물은 {n_zero}/{n} 뿐이다. Ro5 가 걸러내지 못한
이유는 위반이 드물어서가 아니라 **게이트가 위반 1개를 허용하기 때문**이다. 위반 0개를
요구했다면 {n - n_zero}/{n} 이 탈락해 QED 보다 강한 필터가 되었을 것이다.
...
3. 화합물 {n}건은 단일 화학형 계열에 가깝다. 원인은 정렬 미지정보다 §2.2 의 세 제외
...
{refs}
"""
    md = md.replace("{gatelog}", "\n".join(log)).replace("{negctl}", neg)
    out = Path(a.out); out.parent.mkdir(parents=True, exist_ok=True); out.write_text(md)
    print(f"{out}  ({len(md.split())} words)")
    print(f"  화합물 {n} · 통과 {npass} · MW>500 {n_mw_over} · 위반0 {n_zero} · 스킬 {n_skills}")
