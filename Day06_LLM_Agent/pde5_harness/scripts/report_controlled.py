#!/usr/bin/env python3
"""골격 통제 연구 보고서 — 모든 수치를 sample_run/ 산출 파일에서 주입한다.

결론 문장은 데이터에 따라 분기한다. 신호가 골격 통제 후에도 남으면 그렇게 쓰고,
사라지면 사라졌다고 쓴다. 사람이 미리 정해 둔 결론에 수치를 맞추지 않는다.
"""
...
def res(env):
    return None if env is None else env.get("result", env)


def gate(env):
    if not env: return "N/A"
    v = env.get("verification")
    return "N/A" if not v else ("PASS" if v.get("passed") else "FAIL")


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", default=str(SR / "report" / "report_controlled.md"))
    a = ap.parse_args()

    dsE, dkE, anE = load("dataset_controlled.json"), load("docking_controlled.json"), \
                    load("analysis_controlled.json")
    tcE, swE = load("terms_controlled.json"), load("exhaustiveness_sweep.json")
    ...
    protE = load("protonation_test.json"); prot = res(protE)
    csE2 = load("custom_scoring_controlled.json"); cs2 = res(csE2)
    csE = load("custom_scoring_controlled.json")
    cs = res(csE)
    tgtE = load("envelopes/target-lookup.json"); envE = load("environment.json")
    if not (dsE and dkE and anE):
        raise SystemExit("골격 통제 산출 파일이 없다 — 보고서를 만들지 않는다.")
    ds, dk, an = res(dsE), res(dkE), res(anE)
    tc, sw, conc, cust = res(tcE), res(swE), res(concE), res(custE)
    T = an["arms"]["top_pose"]; ctrl = dk["control_redock"]
    EV = res(envE) or {}; TOOLS = EV.get("external_tools", {})
    RECP = EV.get("receptor_preparation", {}); LIGP = EV.get("ligand_preparation", {})
    TG = res(tgtE) or {}
    n = an["n"]; rows = dk["rows"]

    # ── 데이터에서 결론을 유도한다 ────────────────────────────────────
    raw, par = T["spearman"], T["partial_spearman_controlling_tanimoto"]
    wb = T["within_similarity_bin"]
    within_sig = [b for b, v in wb.items() if v.get("perm_p") is not None and v["perm_p"] < .05]
    keep = abs(par) >= 0.6 * abs(raw) and T.get("partial_perm_p", 1) < .05
    share = T.get("confound_share")
    auc_all = T["auc_strong_vs_weak"]
    aucb = T["auc_within_similarity_bin"]
    aucb_s = " · ".join(f"{k} {v}" for k, v in aucb.items())   # dict repr 이 본문에 새지 않게
    PV = T["pose_validity"]; PVI = PV["interpretable_only"]
    PV = T["pose_validity"]; PVI = PV["interpretable_only"]
    PV_EX = ", ".join(PV["excluded_bands"]) or "없음"
    WCTL = T["within_bin_partial"]; ACI = T["auc_ci95"]
    ...
    _pby = {x["condition"]: x for x in (prot or {}).get("summary", [])}
    _base_c2 = _pby.get("recpH74_ligneutral", {}).get("c2_top_rmsd_mean", float("nan"))
    _match_c2 = _pby.get("recpH74_ligpH74", {}).get("c2_top_rmsd_mean", float("nan"))
    _sv = res(statE) or {}
    ...
    _base = sum(1 for r in rows if r["potency_bin"] == "strong") / n
    ESEL_EF = (round((sum(1 for r in _ord10 if r["potency_bin"] == "strong") / _k10) / _base, 2)
               if _base else "—")
    topk = next((k for k in T if k.startswith("top") and k.endswith("_similarity_bins")), None)
    TOPSIM = T.get(topk, {}); TOPPOT = T.get(topk.replace("similarity", "potency"), {}) if topk else {}
    kN = int(topk[3:].split("_")[0]) if topk else 0
    pv_tbl = "\n".join(
        f"| {b} | {PV[b]['n']} | {PV[b]['mean_mcs_atoms']} | {PV[b]['n_at_mcs_floor']} | "
        + (f"{PV[b]['median_rmsd']:.2f} | {PV[b]['frac_under_threshold']:.1%} |"
           if PV[b]["interpretable"] else "— | **해석 불가** |")
        for b in ("far", "mid", "near") if PV.get(b))
    wctl_tbl = "\n".join(
        f"| {b} | {v['n']} | {v['raw']:+.3f} | {v['controlling_size']:+.3f} | "
        f"{v['controlling_tanimoto']:+.3f} | {v['controlling_both']:+.3f} |"
        for b, v in WCTL.items())
    auc_tbl = "\n".join(
        f"| {b} | {v['n_pos']}/{v['n_neg']} | {v['auc']} | {v['ci95'][0]:.3f}–{v['ci95'][1]:.3f} | "
        f"{MUL['bonferroni'].get(b, '—')} |"
        for b, v in ACI.items())
    auc_bins_ok = [b for b, v in aucb.items() if v is not None and v >= .7]
    q2_single = tc["models"]["single_vina_score"]["Q2_loo"] if tc else None
    q2_five = tc["models"]["five_terms"]["Q2_loo"] if tc else None
    q2_tan = tc["models"]["tanimoto_only"]["Q2_loo"] if tc else None
    quant_ok = q2_single is not None and q2_single >= 0.3

    raw_sig = (T.get("perm_p") is not None) and T["perm_p"] < 0.05
    near_bin = wb.get("near", {})
    near_sig = near_bin.get("perm_p") is not None and near_bin["perm_p"] < 0.05
    other_sig = [b for b in ("far", "mid")
                 if wb.get(b, {}).get("perm_p") is not None and wb[b]["perm_p"] < 0.05]

    if not raw_sig:
        # 원상관 자체가 귀무를 못 벗어난다 — "신호가 줄었다"가 아니라 "신호가 없다"이다.
        verdict_head = ("**이 집합에서는 전체 순위 신호 자체가 없다** "
                        f"(ρ = {raw:+.3f}, 95% CI {T['ci95'][0]:+.3f}–{T['ci95'][1]:+.3f}, "
                        f"순열 p = {T['perm_p']}). 앞선 n=30 설계에서 얻은 상관은 "
    ...
    elif abs(par) >= 0.3 * abs(raw):
        verdict_head = ("도킹 점수의 순위 신호는 골격 유사성을 통제하자 **상당 부분 줄었으나 "
                        "완전히 사라지지는 않았다**. 관찰된 선별 성능은 역가와 골격 닮음이 "
                        "함께 만든 것이다.")
    else:
        verdict_head = ("도킹 점수의 순위 신호는 골격 유사성을 통제하자 **대부분 사라졌다**. "
                        "관찰된 선별 성능은 주로 공결정 리간드와의 골격 닮음을 반영한 것이며, "
                        "역가 예측력의 증거로 볼 수 없다.")

    ts = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d")
    cells = "\n".join(f"| {c['potency']} | {c['similarity']} | {c['available']} | {c['taken']} |"
                      for c in ds["cells"])
    wb_tbl = "\n".join(
        f"| {b} | {v['n']} | {v['pIC50_range'][0]:.2f}–{v['pIC50_range'][1]:.2f} | "
        f"{v['spearman']:+.3f} | {v['ci95'][0]:+.3f}–{v['ci95'][1]:+.3f} | {v['perm_p']} | "
        f"{aucb.get(b, '—')} |"
        for b, v in wb.items())
    top_rows = "\n".join(
        f"| {r['chembl_id']} | {r['potency_bin']}/{r['similarity_bin']} | "
        f"{r['pchembl_value']:.2f} | {r['tanimoto_to_sildenafil']:.3f} | "
        f"{r['top_pose_score']:.2f} | {r.get('top_pose_mcs_rmsd') or '—'} |"
        for r in sorted(rows, key=lambda x: x["top_pose_score"])[:15])
    sw_tbl = "\n".join(
        f"| {s['exhaustiveness']} | {s['c1_best_rmsd_mean']:.2f} | {s['c2_top_rmsd_mean']:.2f} | "
        f"{s['top_score_mean']:.2f} | {s['c1_pass_rate']:.2f} | {s['c2_pass_rate']:.2f} |"
        for s in sw["summary"]) if sw else ""
    mdl_tbl = "\n".join(
        f"| {nm} | {m['n_params']} | {m['R2_fit']:+.3f} | {m['Q2_loo']:+.3f} | "
        f"{m['null_R2_median']:+.3f} | {m['null_R2_p95']:+.3f} |"
        for nm, m in tc["models"].items()) if tc else ""
    term_tbl = "\n".join(
        f"| {t} | {tc['standardized_coefficients'][t]:+.3f} | "
        f"{tc['univariate'][t]['pearson_r']:+.3f} | "
        f"{tc['univariate'][t]['pearson_r_vs_tanimoto']:+.3f} | "
        f"{tc['permutation_importance'][t]['mean_Q2_drop']:+.4f} | {tc['vif'][t]} |"
        for t in tc["short"]) if tc else ""
    conc_tbl = "\n".join(f"| {k} | {v['selected']}/{v['of_n']} | {v['top']}/{v['of_n']} |"
                         for k, v in (conc or {}).get("key_residue_frequency", {}).items())
    F = "figures_controlled"
    if prot:
        _by = {x["condition"]: x for x in prot["summary"]}
        _base = _by.get("recpH74_ligneutral", {})
        _match = _by.get("recpH74_ligpH74", {})
        _LBL = {"recpH74_ligpH74": "수용체 pH7.4 · 리간드 pH7.4 **(옳은 짝)**",
                "recpH74_ligneutral": "수용체 pH7.4 · 리간드 중성 **(현행)**",
                "recneutral_ligneutral": "수용체 중성 · 리간드 중성",
                "recneutral_ligpH74": "수용체 중성 · 리간드 pH7.4"}
        _ptbl = "\n".join(
            "| {} | {:.2f} | {:.0%} | {:.2f} | {:.0%} | {:.2f} |".format(
                _LBL.get(x["condition"], x["condition"]), x["c1_best_rmsd_mean"],
                x["c1_pass_rate"], x["c2_top_rmsd_mean"], x["c2_pass_rate"],
                x["top_score_mean"])
            for x in prot["summary"])
        _delta = prot.get("c2_change_matched_minus_baseline") or 0.0
        _M = prot.get("mechanism", {})
        prot_section = f"""### 3.2b 프로토네이션 비대칭이 C2 실패의 원인인가
...
{prot['ligand_formal_charge']['lig_neutral']} 에서
{prot['ligand_formal_charge']['lig_pH74']} 로 바뀐다 — **중성 리간드를 pH 7.4 수용체에 넣은
것이 C2 실패의 직접 원인일 수 있다.** 이전 판은 이를 한계로만 적고 검정하지 않았다.

수용체 {{pH 7.4, 중성}} × 리간드 {{중성, pH 7.4}} 의 2×2 를 시드
{prot['seeds']} 로 돌렸다. RMSD 는 중원자 기준이라 프로토네이션이 지표 자체를 바꾸지 않으므로
조건 간 비교가 깨끗하다.
...
**{prot['verdict']}.** 물리적으로 옳은 짝으로 바꿨을 때 C2 는
{_base.get('c2_top_rmsd_mean', float('nan')):.2f} Å 에서
{_match.get('c2_top_rmsd_mean', float('nan')):.2f} Å ({_delta:+.2f} Å) 이고 통과율은
{_base.get('c2_pass_rate', 0):.0%} → {_match.get('c2_pass_rate', 0):.0%} 다. 최고 점수는 네
조건 모두 {_match.get('top_score_mean', float('nan')):.2f} kcal/mol 로 동일하다.

**왜 아무 일도 일어나지 않았는지까지 확인했다.** 관찰만 적으면 "이 표적에서는 우연히
영향이 없었다"로 읽힐 수 있어 기전을 따로 검사했다 ({_M['explanation'].split('.')[0]}…).

| 확인 항목 | 결과 |
|---|---|
| 리간드 원자 타이핑이 실제로 바뀌었나 | {'예' if _M['typing_did_change'] else '아니오'} — 피페라진 질소가 수용체(NA)에서 공여체(N)로, 극성 H +1 |
| 수소결합 항이 움직였나 | {'아니오' if _M['hbond_term_unchanged'] else '예'} (중성 {_M['terms_neutral']['hbond']:.5f} vs pH7.4 {_M['terms_pH74']['hbond']:.5f}) |
| 채점 함수에 정전기 항이 있나 | {'예' if _M['has_electrostatic_term'] else '아니오'} |
| 다른 화합물을 넣으면 항이 반응하나 | {'예' if _M['sanity_other_ligand_differs'] else '아니오'} (위생 검사) |
...
"""
        ...
        prot_section = "*(프로토네이션 검정은 이 실행에 포함되지 않았다.)*"

    md = f"""# 골격 유사성과 역가를 직교화한 PDE5A 도킹 재평가
...
**작성일** {ts} · **버전** 3.0 · **원자료** `sample_run/`
(dataset `{sha16(SR/'dataset_controlled.json')}`, docking `{sha16(SR/'docking_controlled.json')}`,
analysis `{sha16(SR/'analysis_controlled.json')}`)

![그래픽 초록]({F}/fig01_graphical_abstract.png)
...
**방법.** PDE5A 결정 구조(PDB {dk['receptor_pdb'].replace('.pdb','')}, 공결정 리간드
{dk['reference_ligand']})를 표적으로 삼았다. ChEMBL 에서 {ds['records_scanned']}개 활성
레코드를 받아 화합물당 중앙값으로 집계한 뒤({ds['pool_size']}종), **역가 3구간 × 공결정
리간드와의 Tanimoto 유사도 3구간의 9칸 격자**에서 균등 추출해 {n}종을 만들었다. 이 설계로
두 축의 상관을 r={ds['confound_pearson_r_potency_vs_similarity']:+.3f} 까지 낮췄다.
{dk['engine']} 으로 도킹하고, 공결정 리간드 재도킹을 대조로 두되 **샘플링(C1)과 채점(C2)을
따로 판정**했다. 자세는 공결정 리간드를 참조하지 않는 **점수 1위 자세**를 주 결과로 썼다.

**결과.** 대조에서 샘플링은 결정 자세를 재현했으나({ctrl['rmsd_best_of_modes_angstrom']} Å,
모드 {ctrl['best_mode_index']}) 채점은 그 자세를 1위로 올리지 못했다
({ctrl['rmsd_top_pose_angstrom']} Å). 탐색 깊이를 {sw['levels'][0] if sw else '—'}에서
{sw['levels'][-1] if sw else '—'}까지 32배 올리자 C1 은 통과율 0%→100% 로 완전히 고쳐진 반면
C2 는 전 구간 0% 이고 최고 점수는 변하지 않았다 — 탐색은 얕은 깊이에서 이미 전역 최소를
찾았고 그 최소가 틀린 자리에 있다. 전체 순위상관은 {raw:+.3f}
(95% CI {T['ci95'][0]:+.3f}–{T['ci95'][1]:+.3f}, 순열 p={T['perm_p']}) 였고, **Tanimoto 를
통제한 편상관은 {par:+.3f}** (p={T['partial_perm_p']}) 였다. 유사도 구간 안에서 유의한
상관을 보인 구간은 {len(within_sig)}/3 개였다. 강함/약함 판별 ROC-AUC 는 전체 {auc_all},
구간별 {aucb_s}. 교차검증 Q² 는 단일 점수 {q2_single}, 5개 항 {q2_five}, Tanimoto 단독 {q2_tan} 였다.

**결론.** {verdict_head} 정량 예측은 사전 기준(Q² ≥ 0.3)을
{'충족했다' if quant_ok else '충족하지 못했다'}.

**이 판은 자기 초안의 설명도 함께 기각한다.** v2.0 은 n=30 에서
ρ={col['old_design']['spearman_top_pose']:+.3f} 를 얻었고 본 보고서 초안은 그것이 골격 교란의
산물이라고 적었다. 그러나 같은 편상관을 옛 데이터에 적용하면 감쇠는
**{col['old_design']['attenuation_from_scaffold_control']:.1%}** 에 불과하고, 옛 데이터의
...
![Fig 2]({F}/fig02_dataset.png)

**Figure 2. 데이터셋 특성.** (A) 역가 분포, (B) 골격 유사도 분포, (C) 두 축의 직교성,
(D) 화합물당 반복 측정 수.

표적은 PDE5A 다{f" (UniProt {TG['accession']}, {TG['gene']}, {TG['length']} 잔기)" if TG else ""}.
수용체는 PDB {dk['receptor_pdb'].replace('.pdb','')}, 공결정 리간드는 {dk['reference_ligand']}
(실데나필)이다.
...
**Table 1. 9칸 격자별 추출.** 최종 n={n}. `weak × near` 칸이 얇은 것은 화학적으로 당연하다
— 실데나필과 매우 닮았는데 약한 화합물은 드물다. 이 불균형이 남은 잔여 교란의 원인이며
§4.4 에서 다룬다.

결과적으로 역가와 유사도의 상관은 Pearson **r = {CM['pearson_r_potency_vs_similarity']:+.3f}**,
Spearman **ρ = {CM['spearman_rho_potency_vs_similarity']:+.3f}** 로 낮아졌다. 두 지표를 모두
...
{CM['spearman_tanimoto_vs_heavy_atoms']:+.3f} 이므로, "골격"과 "크기"라는 두 교란축은
서로 상당히 겹친다. 둘을 동시에 통제해도 독립적인 두 통제가 아니다.

### 2.3 도킹

**도구.** {TOOLS.get('smina','—')} · {TOOLS.get('open_babel','—')} · PyMOL {TOOLS.get('pymol','—')}.

**수용체 준비.** {RECP.get('source','—')} 에서 물({RECP.get('waters_removed','—')})을 제거하고
촉매 금속 {', '.join(RECP.get('metals_kept',[])) or '—'} 은 남겼다. 공결정 리간드를 분리해
상자 기준으로 썼다. PDBQT 변환은 `{RECP.get('pdbqt_conversion','—')}` — 프로토네이션은
수용체에만 적용했다.
...
**도킹.** {dk['engine']}, seed {dk['seed']}, `--num_modes 9`, `--autobox_add 3`.
{dk['n_docked']}/{dk['n_attempted']}건 성공.

**자세 선택.** {dk['pose_selection']}
...
올리는가. 기준은 {ctrl['threshold_angstrom']} Å 이다. 두 대조를 합쳐서 보면 하나의 실패가
다른 하나로 오진된다.
...
n={n}, pChEMBL {min(r['pchembl_value'] for r in rows):.2f}–{max(r['pchembl_value'] for r in rows):.2f}
(약 {max(r['pchembl_value'] for r in rows) - min(r['pchembl_value'] for r in rows):.1f} 로그),
Tanimoto {min(r['tanimoto_to_sildenafil'] for r in rows):.3f}–{max(r['tanimoto_to_sildenafil'] for r in rows):.3f}.
역가-유사도 상관 r={ds['confound_pearson_r_potency_vs_similarity']:+.3f} (Figure 2C).

### 3.2 대조 — 병목은 채점이다

![Fig 3]({F}/fig03_control.png)
...
| C1 샘플링 (상위 모드 최선 RMSD) | {ctrl['rmsd_best_of_modes_angstrom']} Å (모드 {ctrl['best_mode_index']}) | {'PASS' if ctrl['sampling_control_passed'] else 'FAIL'} |
| C2 채점 (1위 자세 RMSD) | {ctrl['rmsd_top_pose_angstrom']} Å | {'PASS' if ctrl['ranking_control_passed'] else 'FAIL'} |

**Table 2. 재도킹 대조.** 기준 {ctrl['threshold_angstrom']} Å.

탐색 깊이를 바꿔 같은 대조를 반복했다.

![Fig 4]({F}/fig08_exhaustiveness.png)
...
{sw['diagnosis']['c2_range_angstrom']} Å 밖에 변하지 않고 통과율은 **모든 깊이에서 0%** 다.

**셋째, 점수는 가장 얕은 깊이에서 이미 포화한다.** 최고 점수는 전 구간에서
{sw['diagnosis']['score_range_kcal']} kcal/mol 변동, 즉 사실상 상수다.

세 관찰을 합치면 결론은 하나다. **탐색은 깊이 {sw['levels'][0]} 에서 이미 채점 함수의 전역
최소를 찾아냈고, 그 최소가 결정 자세에서 {sw['summary'][-1]['c2_top_rmsd_mean']:.1f} Å 떨어진
곳에 있다.** 채점 함수의 최적점 자체가 틀린 위치에 있으므로 탐색을 아무리 늘려도 고쳐지지
않는다. 실제로 이 표에서 깊이를 {sw['levels'][0]} 에서 {sw['levels'][-1]} 로 32배 늘렸을 때
...
![Fig 5]({F}/fig04_score_vs_potency.png)

**Figure 5. 도킹 점수 대 실측 역가.** 왼쪽은 전체, 나머지는 유사도 구간별.

전체 순위상관은 **{raw:+.3f}** (95% CI {T['ci95'][0]:+.3f}–{T['ci95'][1]:+.3f},
순열 p={T['perm_p']}) 다. 점수는 낮을수록, pChEMBL 은 높을수록 좋으므로 음의 상관이 양의
예측력을 뜻한다.

그런데 같은 데이터에서 **점수와 Tanimoto 의 상관은 {T['spearman_score_vs_tanimoto']:+.3f}**,
**역가와 Tanimoto 의 상관은 {T['spearman_pIC50_vs_tanimoto']:+.3f}** 다. 세 상관이 모두
...
|---|---|
| 없음 (원상관) | {raw:+.3f} |
| 골격 유사도 (Tanimoto) | {par:+.3f} (순열 p={T['partial_perm_p']}) |
| 분자 크기 (중원자수) | {T['partial_spearman_controlling_size']:+.3f} |
| **골격 + 크기 동시** | **{T['partial_spearman_controlling_both']:+.3f}** |

**Table 6. 교란 통제 단계별 편상관.** 점수와 중원자수의 상관은
{T['spearman_score_vs_heavy_atoms']:+.3f}, 역가와 중원자수의 상관은
{T['spearman_pIC50_vs_heavy_atoms']:+.3f} 다. Vina 계열 점수가 분자 크기에 반응한다는 것은
알려진 성질이므로 [R5] 골격만 통제하면 부족하다.

Tanimoto 를 통제한 편상관은 **{par:+.3f}** (순열 p={T['partial_perm_p']}) 다. 원상관
{raw:+.3f} 보다 절댓값이 **{abs(par)/abs(raw):.2f}배로 커졌다** — 줄어든 것이 아니다.
Tanimoto 가 pIC50 과 양(+{T['spearman_pIC50_vs_tanimoto']:.3f})이면서 점수와도
양(+{T['spearman_score_vs_tanimoto']:.3f})이라, 통제하면 가려져 있던 약한 음의 관계가
드러나는 **억제변수(suppressor)** 구조다. 다만 커진 뒤에도 유의하지 않으므로
(p={T['partial_perm_p']}) 결론은 바뀌지 않는다.
유사도 구간 안에서 유의한 상관(p<0.05)을 보인 구간은 **{len(within_sig)}/3**
({', '.join(within_sig) if within_sig else '없음'}) 이다.
구간내 상관의 평균은 {T['within_bin_mean_spearman']:+.3f} 다.
...
({WCTL['near']['raw']:+.3f} → {WCTL['near']['controlling_both']:+.3f}). far 대역은 크기 통제
시 **부호가 뒤집힌다** ({WCTL['far']['raw']:+.3f} → {WCTL['far']['controlling_size']:+.3f}).
전역 편상관 하나로는 이 이질성이 보이지 않는다.

**near 대역 신호는 한 칸에 얹혀 있다.** 그 대역의 weak 칸은 풀 전수(census)이고 pChEMBL 이
어세이 바닥값에 몰려 있다. 그 칸을 빼면 상관은 {NSEN['drop_weak_cell']['spearman']:+.3f}
(n={NSEN['drop_weak_cell']['n']}) 로 떨어진다. 바닥값 화합물만 빼면
{NSEN['drop_pchembl_5.00']['spearman']:+.3f} (n={NSEN['drop_pchembl_5.00']['n']}) 다.
**따라서 near 대역 결과는 잠정적으로 읽어야 한다.**

이중 회귀(표준화)에서 점수 계수는
{T['dual_regression_standardized']['beta_score']:+.3f}, Tanimoto 계수는
{T['dual_regression_standardized']['beta_tanimoto']:+.3f} 였다.

### 3.5 앞선 판의 상관은 왜 무너졌는가 — 세 가지를 검정했다

v2.0 은 n=30 에서 ρ={col['old_design']['spearman_top_pose']:+.3f}
(p={col['old_design']['perm_p']}) 를 보고했다. 본 보고서의 초안은 그 값이 골격 교란의
산물이라고 적었으나, **검정해 보니 그 설명은 성립하지 않았다.**

| 검정 | 결과 | 해석 |
|---|---|---|
| ① 옛 데이터에서 Tanimoto 통제 | {col['old_design']['spearman_top_pose']:+.3f} → {col['old_design']['partial_controlling_tanimoto']:+.3f} (감쇠 {col['old_design']['attenuation_from_scaffold_control']:.1%}) | 골격 교란이 아니다 |
...
{PVI['frac_under_threshold']:.1%} 보다도 낮다. **이 연구에서 가장 강한 상관이 자세가
...
{ctrl['threshold_angstrom']} Å 기준(§3.2 의 C2 와 동일)으로 1위 자세를 평가했다.

| 유사도 대역 | n | MCS 원자수 평균 | 8원자 하한 건수 | RMSD 중앙값 (Å) | 2 Å 기준 통과 |
|---|---|---|---|---|---|
{pv_tbl}
| **해석 가능 대역 합** | **{PVI['n']}** | **{PVI['mean_mcs_atoms']}** | **{PVI['n_at_mcs_floor']}** | **{PVI['median_rmsd']:.2f}** | **{PVI['frac_under_threshold']:.1%}** |
| (참고) far 포함 전체 | {PV['all']['n']} | {PV['all']['mean_mcs_atoms']} | {PV['all']['n_at_mcs_floor']} | {PV['all']['median_rmsd']:.2f} | {PV['all']['frac_under_threshold']:.1%} |
...
를 잰다. 같은 결합 양식을 전제하는 셈이다.

**그래서 far 대역은 값을 적지 않았다.** 그 대역의 공유 부분구조는 평균
{PV['far']['mean_mcs_atoms']}원자이고 {PV['far']['n']}건 중 {PV['far']['n_at_mcs_floor']}건이
하한 8원자에 걸려 있다. 원자 열 개가 겹치는 위치를 재서 "자세가 맞는가"를 판정할 수는 없다.
해석할 수 없다고 적은 값을 그래도 인용하면 독자가 그 숫자를 쓰게 되므로 아예 비웠다.

**같은 이유로 헤드라인은 해석 가능한 대역(mid·near)만으로 낸다.** far 를 통합값에 넣으면
그 통합값도 오염된다. 초안은 far 를 포함한 {PV['all']['frac_under_threshold']:.1%} 를 헤드라인으로
썼는데, 그것을 {PVI['frac_under_threshold']:.1%} 로 정정한다. 참고를 위해 두 값을 모두 표에 남겼다.

**해석 가능한 대역에서도 자세의 {1 - PVI['frac_under_threshold']:.0%} 가 C2 대조를 실패시킨 것과
같은 기준을 넘지 못한다** (n={PVI['n']}). 즉 §3.4 의 "신호 없음"은 **대부분 결정 자세 틀 밖에
놓인 자세에 매긴 점수**에 대한 진술이고, near 대역의 유의한 상관도
{PV['near']['frac_under_threshold']:.0%} 만 틀 안에 있는 자세들에서 나왔다. 이것이 본 연구의
가장 큰 해석 제약이며 §4.1 에서 다시 다룬다.

평균 점수는 far 가 가장 좋다 ({MSB['far']:.2f} vs near {MSB['near']:.2f}). 점수 상위
{kN}건의 대역 구성은 far {TOPSIM.get('far','—')} · mid {TOPSIM.get('mid','—')} ·
near {TOPSIM.get('near','—')} (무작위 기대 각 {kN/3:.1f}), 역가 구성은
강 {TOPPOT.get('strong','—')} · 중 {TOPPOT.get('medium','—')} · 약 {TOPPOT.get('weak','—')} 이다.
도킹 점수는 0.1 kcal/mol 해상도로 출력되어 {n} 건에 서로 다른 값이 소수뿐이며, 동점이 많아
상위 k 구성은 정렬 안정성에 따라 한두 건 달라질 수 있다.

### 3.7 선별 지표

![Fig 7]({F}/fig06_roc.png)

**Figure 7. ROC 곡선.** (A) 전체, (B) 유사도 구간별.

![Fig 8]({F}/fig07_enrichment.png)
...
3개 대역 상관 검정에 대한 다중비교 보정은 Bonferroni {MUL['bonferroni']['near']} ·
BH {MUL['benjamini_hochberg']['near']} 로, **near 대역은 보정 후에도 유의하다.**
(§4.4 한계 8 의 "미보정" 서술을 이 결과로 대체한다.)

농축계수는 상위 10% 에서 {ESEL_EF} 다.

### 3.8 스코어 항 분석

![Fig 9]({F}/fig09_term_importance.png)

**Figure 9. 항 기여도 네 가지 관점.** 지표마다 다른 답을 준다면 그 사실이 결과다.

![Fig 10]({F}/fig10_terms_models.png)
...
(R² {tc['models']['tanimoto_only']['R2_fit']:+.3f} vs 섞기 95%
{tc['models']['tanimoto_only']['null_R2_p95']:+.3f})이고, Q² 가 음수가 아닌 것도 그것뿐이다
({tc['models']['tanimoto_only']['Q2_loo']:+.3f}). 즉 **이 데이터에서는 골격 유사성 하나가
...
{tc['permutation_importance'][tc['short'][0]]['n_shuffles']}회 셔플로만 추정했고 표준오차를
내지 않았으며, 그 대상 모델(5개 항)의 R² 는 위에서 보았듯 우연 수준을 넘지 못한다. 우연과
구별되지 않는 모델 안에서 항의 상대적 기여를 논하는 것은 잡음을 읽는 일이다.

### 3.9 자세 선택 규칙 민감도

![Fig 11]({F}/fig11_pose_rule.png)

**Figure 11. 두 자세 규칙 비교.** (A) 공결정 리간드와의 거리 분포, (B) 상관 비교.

주 결과는 공결정 리간드를 참조하지 않는 점수 1위 자세다
({raw:+.3f}). 참조 기준으로 고른 자세를 쓰면
{an.get('sensitivity_reference_pose', {}).get('spearman', '—')} 이다. 결론은 규칙에 의존하지 않는다.

### 3.10 적합 가중치를 실제 채점 함수로 넣어 재도킹

{f'''§3.8 의 회귀는 이미 도킹된 자세를 사후 재채점한 것이다. 가중치가 정말 유용하다면
**도킹 자체를 그 가중치로 다시 돌렸을 때** 더 나은 순위가 나와야 한다. 적합 가중치를
smina `--custom_scoring` 파일로 내보내고, 9칸 격자에서 **층화 분할**한 시험 집합
{cs["n_scored"]}건을 두 번 도킹했다 — 한 번은 기본 Vina 점수로, 한 번은 적합 가중치로.
화합물·수용체·상자·시드가 모두 같으므로 차이는 채점 함수뿐이다. 가중치는 훈련 집합
{cs["n_train"]}건에서만 적합했고 시험 집합은 적합에 쓰이지 않았다.

![Fig 12]({F}/fig13_custom_scoring.png)
...
|---|---|---|
| 기본 Vina | {cs["arm_default"]["spearman"]:+.3f} | {cs["arm_default"]["ci95"]} |
| 적합 가중치 | {cs["arm_custom"]["spearman"]:+.3f} | {cs["arm_custom"]["ci95"]} |

**Table 13. 독립 시험 집합에서의 채점 함수 비교** (n={cs["n_scored"]}, 훈련 {cs["n_train"]}건과 분리).

차이는 {cs["difference"]["delta_spearman"]:+.3f}, 부트스트랩 95% 신뢰구간은
[{cs["difference"]["bootstrap_ci95"][0]:+.3f}, {cs["difference"]["bootstrap_ci95"][1]:+.3f}]
이고 커스텀이 더 나은 재표본의 비율은
{cs["difference"]["frac_resamples_favoring_custom"]:.1%} 다.
{"신뢰구간이 0 을 포함하므로 개선을 주장할 수 없다." if cs["difference"]["bootstrap_ci95"][0] <= 0 <= cs["difference"]["bootstrap_ci95"][1] else "신뢰구간이 0 을 배제하므로 차이는 우연으로 보기 어렵다."}

**점 추정값만 보고했다면 다른 결론을 썼을 것이다.** 숫자가 원하는 방향으로 움직였는지와
그 움직임이 우연과 구별되는지는 다른 질문이며, 후자에는 신뢰구간이 필요하다.''' if cs else "*(커스텀 채점 검증은 이 실행에 포함되지 않았다.)*"}

### 3.11 결합 양상과 접촉 잔기

{f'''![Fig 13a]({F}/fig7_binding_overview.png)

**Figure 13. 결합 부위와 자세.** (위) 수용체 카툰과 포켓 위치, (아래) 포켓 근접 — 결정
자세(회색)와 **점수 1위 도킹 자세**(색), 극성 접촉. 이 자세는 공결정 리간드를 참조하지 않고
채점 함수만으로 뽑힌 것이다.

![Fig 13b]({F}/fig8_binding_pocket.png)
...
{sw['levels'][0] if sw else '—'}–{sw['levels'][-1] if sw else '—'} × 시드 {len(sw['seeds']) if sw else '—'}개의
통제 실험으로 대체했다 (§3.2).

**탐색을 32배 늘렸을 때 C1 은 완전히 고쳐지고(통과율 0% → 100%) C2 는 전혀 움직이지 않았다
(통과율 0% → 0%, 변동 {sw['diagnosis']['c2_range_angstrom']} Å).** 최고 점수가 가장 얕은
...
1 → {len(sw['levels']) * len(sw['seeds']) if sw else '—'} 로 늘었지만 화학적 표본은 여전히
n=1 이다. 따라서 이 실험이 지지하는 진술은 **"탐색 깊이는 병목이 아니다"** 까지이며,
"채점 함수가 원인이다" 는 그보다 넓은 주장이다.

**가장 큰 제약은 자세 타당도다** (§3.6). 분석 대상 자세의
{1 - PVI['frac_under_threshold']:.0%} 가 2 Å 기준을 넘지 못한다 (해석 가능 대역
{PVI['n']}건 기준). C2 대조가 실패했다는
...
{_base_c2:.2f} → {_match_c2:.2f} Å 로 움직이지 않는다. 기전까지 확인했다 — 기본 Vina 함수에
...
far 대역은 결정 자세 틀 안에 놓이는 비율이 해석 자체가 불가능하므로,
"닮지 않은 분자가 상자 안에서 접촉을 최대화하는 임의 자세를 찾아 좋은 점수를 얻는다"는 설명이
가능하지만 **검정하지 않았다.**

**이 보고의 기여는 기전 설명이 아니라 두 가지 음성 결과다.**

첫째, **더 크고 골격을 통제한 표본에서 도킹 점수는 역가와 관계가 없다** (ρ={raw:+.3f},
95% CI {T['ci95'][0]:+.3f}–{T['ci95'][1]:+.3f}). 단 이 "없다"는 검정력 한계 안에서의
진술이며 |ρ| ≲ {MDE['전체']} 인 관계는 배제하지 못한다 (§4.4 한계 9).
...
{col['old_design']['attenuation_from_scaffold_control']:.1%} 로 거의 0 이었다.** 이 사실을
적지 않고 처방만 남기면 앞선 판이 저지른 것과 같은 종류의 과잉 주장이 된다.

### 4.3 접촉 잔기 일치는 검증력이 거의 없다

{f'''핵심 잔기는 참조 기준 자세와 점수 1위 자세 **양쪽 모두**에서 전 화합물이 접촉한다.
...
{ctrl['rmsd_top_pose_angstrom']} Å 벗어난 1위 자세조차 같은 잔기와 접촉한다.

**"알려진 잔기와 접촉한다"는 흔한 정당화 문장은 결합 부위를 지정해 도킹하면 거의 자동으로
참이 된다.** 자세를 검증하려면 실험 좌표와의 RMSD 처럼 틀릴 수 있는 지표를 써야 한다.''' if conc else "*(이 실행에는 접촉 분석이 포함되지 않았다.)*"}
...
4. **9칸이 균형이 맞지 않는다.** `weak × near` 칸의 풀 후보가 적어 (Table 1) 잔여 교란
   r={ds['confound_pearson_r_potency_vs_similarity']:+.3f} 가 남았다. 완전한 직교화는 화학적으로
...
12. **자세 타당도가 낮다** (§3.6). 분석 자세의
   {1 - PVI['frac_under_threshold']:.0%} 가 2 Å 기준 밖이며(해석 가능 대역 기준),
   이것이 모든 상관 해석의
   상한을 정한다.
...
**탐색 깊이는 병목이 아니다.** 깊이를 {sw['levels'][0] if sw else '—'}에서
{sw['levels'][-1] if sw else '—'}로 32배 올리자 C1 은 통과율 0%→100% 로 고쳐졌고 C2 는
전 구간 0%, 최고 점수는 변동 {sw['diagnosis']['score_range_kcal'] if sw else '—'} kcal/mol
...
**정량 예측은 사전 기준을 {'충족했다' if quant_ok else '충족하지 못했다'}.**
교차검증 Q² 는 단일 점수 {q2_single}, 5개 항 {q2_five}, Tanimoto 단독 {q2_tan} 로 모두 0 근처이며
기준 0.3 에 {'도달했다' if quant_ok else '한참 미달한다'}. 이 점수로 개별 IC50 을 추정하는 것은
{'조심스럽게 가능하다' if quant_ok else '정당화되지 않는다'}.

**선별도 전체적으로는 작동하지 않는다.** {verdict_head}

**한 대역에서만 신호가 있고, 그 신호는 취약하다.** 유사도 대역별로 나눠 보면
far {wb.get('far',{}).get('spearman','—')} · mid {wb.get('mid',{}).get('spearman','—')} ·
near {wb.get('near',{}).get('spearman','—')} 이고 유의한 것은 near 대역뿐이다
(p={near_bin.get('perm_p','—')}; Bonferroni {MUL['bonferroni']['near']} 로 보정 후에도 유의).
교란 통제 후 오히려 강해지므로 (→{WCTL['near']['controlling_both']:+.3f}) 골격이나 크기가
만든 값도 아니다.

**그러나 이 결과에 무게를 실을 수 없다.** 세 가지 때문이다. (a) 그 대역의 AUC 신뢰구간
[{ACI['near']['ci95'][0]:.3f}, {ACI['near']['ci95'][1]:.3f}] 이 사전 기준 0.7 을 포함한다.
(b) 전수(census)인 weak 칸을 빼면 상관이 {NSEN['drop_weak_cell']['spearman']:+.3f} 로 떨어져
...
연구에서 그 비율은 {PVI['frac_under_threshold']:.1%} 였고, 이 사실 없이 상관값만
...
| 단계 | 산출 | 게이트 검사 | 결과 |
|---|---|---|---|
| 데이터셋 (골격 통제) | `dataset_controlled.json` | 9칸 충족 · 역가 3로그 · 교란 r < 0.35 · 중복 | {gate(dsE)} |
| 도킹 | `docking_controlled.json` | 성공률 95% · C1 대조 · 전 화합물 점수 · 9칸 대표 | {gate(dkE)} |
| 탐색 깊이 스윕 | `exhaustiveness_sweep.json` | 전 조건 실행 · 4단계 이상 · 복수 시드 | {gate(swE)} |
| 통제 분석 | `analysis_controlled.json` | 상관 · 3구간 · 편상관 · 순열 p | {gate(anE)} |
| 항 분석 | `terms_controlled.json` | 추출률 95% · 4모델 · VIF · 섞기 1000회 | {gate(tcE)} |
| 접촉 일치 | `contact_concordance.json` | 강함 층 전수 · 두 자세 접촉 | {gate(concE)} |
...
| 프로토네이션 2×2 | `protonation_test.json` | 4 조건 실행 · 리간드 전하 실제 차이 · 기전 확인 · 위생 검사 | {gate(protE)} |
...
![Fig 14]({F}/fig12_version_history.png)
...
7. **초안의 인과 주장 철회** — 초안은 v1/v2 의 상관을 골격 교란의 산물로 설명했다. 외부
   ...
   {col['old_design']['attenuation_from_scaffold_control']:.1%} 에 불과했다. 해당 주장을
   철회하고 §3.5 에 검정 기록으로 대체했다.
8. **자세 타당도 보고** — 1위 자세의 {1 - PVI['frac_under_threshold']:.0%} 가 2 Å 기준
...
13. **far 대역 자세 타당도 값 삭제 + 헤드라인 정정** — 리뷰가 "해석할 수 없다고 방금
   말한 숫자를 보여줄 이유가 있나" 를 물었다. far 대역의 공유 부분구조는 평균
   {PV['far']['mean_mcs_atoms']}원자이고 {PV['far']['n']}건 중
   {PV['far']['n_at_mcs_floor']}건이 하한 8원자에 걸려 있어 RMSD 가 자세 정확도를 재지
   못한다. 그 값을 표에서 비웠다. **그리고 같은 이유로 통합 헤드라인도 정정했다** —
   해석 불가한 대역을 통합값에 넣으면 통합값도 오염되므로,
   {PV['all']['frac_under_threshold']:.1%} (far 포함) 대신
   {PVI['frac_under_threshold']:.1%} (mid·near, n={PVI['n']}) 를 쓴다. 즉 "자세의
   {1 - PV['all']['frac_under_threshold']:.0%} 가 기준 밖" 이 아니라
   "{1 - PVI['frac_under_threshold']:.0%} 가 기준 밖" 이다. 해석 가능 여부는
   `analysis_controlled.json` 이 MCS 원자수에서 자동 판정한다.
14. **프로토네이션 검정 실행** — 1·2차 리뷰가 모두 "검정하지 않았다"고 지적한 항목이다.
   2×2 로 돌린 결과 원인이 아니었고, 기본 Vina 함수에 정전기 항이 없어 **원리적으로**
   원인이 될 수 없음까지 확인했다 (§3.2b).
15. **전 그림·문서 재생성** — 본문이 바뀌면 그림과 배포 문서도 바뀌어야 한다.
...
"""
    ...
    print(f"판정: 편상관 {par:+.3f} / 원상관 {raw:+.3f} → "
          f"{'신호 유지' if keep else '신호 감소'}")
    return 0
