...
def main() -> int:
    ...
    md = f"""# 도킹 점수는 역가를 보는가, 골격을 보는가
## 골격 유사성을 설계로 분리한 PDE5A 재채점 연구 (n={n})
...
구조 기반 가상 스크리닝은 표적 결합 부위에 화합물을 계산으로 끼워 넣고 그 점수로
우선순위를 정한다. 이 관행이 성립하려면 세 가지가 따로 참이어야 한다. **자세 예측** —
프로그램이 실제 결합 자세를 만들 수 있어야 한다. **채점** — 그 자세에 매기는 점수가 실측
친화도와 같은 방향으로 움직여야 한다. 그리고 **비교 가능성** — 서로 다른 화합물 사이의
점수 차이가 화학적 유사성이 아니라 결합력의 차이를 반영해야 한다. 세 조건은 독립이며,
앞의 둘이 성립해도 셋째는 실패할 수 있다 [R5, R6].

셋째 조건이 이 보고의 초점이다. 도킹 평가 연구는 보통 활성 화합물 집합을 모아 점수와
역가의 상관을 낸다. 그런데 표적 구조는 대개 **어떤 리간드가 결합한 상태로 풀린 holo
구조**이고, 그 리간드를 닮은 화합물은 (a) 포켓에 기하학적으로 잘 맞아 점수가 좋고 (b) 같은
계열의 최적화 산물이라 대체로 더 강력하다. 두 경로가 같은 방향이므로 상관이 관찰되지만,
그것이 채점 함수의 능력인지 골격 유사성의 부산물인지 구분되지 않는다.

본 보고는 **역가와 골격 유사성을 설계로 직교화**해 이 교란을 제거한다. 그리고 다음 세
질문에 각각 답한다.

1. **정량 예측** — 점수가 pChEMBL 을 얼마나 설명하는가. 교차검증 Q² 로 판정한다.
2. **선별** — 점수가 강한 화합물을 상위로 올리는가. 순위상관·ROC-AUC·농축계수로 판정한다.
3. **교란** — 위 성능이 골격 유사성을 통제한 뒤에도 남는가. 편상관과 구간내 상관으로 판정한다.

**사전 성공 기준.** 정량 예측은 Q² ≥ 0.3 (QSAR 관행), 선별은 순열 p < 0.05 이면서
ROC-AUC ≥ 0.7 을 성공으로 본다. 교란 통제는 편상관의 절댓값이 원상관의 60% 이상 남고
p < 0.05 이면 "살아남았다"로 판정한다. 이 기준의 사후성은 §2.6 에 고지한다.
...
**프로토네이션 비대칭이 특히 직접적인 후보다.** 수용체는 pH 7.4 로 프로토네이션했으나
(`obabel -p 7.4`) 리간드는 중성 `AddHs` 그대로다 (§2.3). 실데나필의 피페라진은 생리적
pH 에서 양성자화되므로, 중성 실데나필을 pH 7.4 수용체에 재도킹한 것이 C2 실패의 직접
원인일 수 있다. 본 연구는 이를 검정하지 않았다. 그 밖의 후보 — 물 전량 제거, 강체 수용체,
단일 배좌, holo 유도적합 편향 — 도 마찬가지다.
