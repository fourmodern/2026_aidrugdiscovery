...
def main() -> int:
    ...
    pv_tbl = "\n".join(
        f"| {b} | {PV[b]['n']} | {PV[b]['median_rmsd']:.2f} | {PV[b]['frac_under_threshold']:.1%} | "
        f"{MSB.get(b, float('nan')):.2f} |"
...
유의한 상관도 {PV['near']['frac_under_threshold']:.0%} 만 틀 안에 있는 자세들에서 나왔다.
...
{1 - PV['all']['frac_under_threshold']:.0%} 가 2 Å 기준을 넘지 못한다. C2 대조가 실패했다는
...
far 대역은 결정 자세 틀 안에 놓이는 비율이 {PV['far']['frac_under_threshold']:.1%} 에 불과하므로,
...
12. **자세 타당도가 낮다** (§3.6). 분석 자세의
   {1 - PV['all']['frac_under_threshold']:.0%} 가 2 Å 기준 밖이며, 이것이 모든 상관 해석의
   상한을 정한다.
...
연구에서 그 비율은 {PV['all']['frac_under_threshold']:.1%} 였고, 이 사실 없이 상관값만
...
8. **자세 타당도 보고** — 1위 자세의 {1 - PV['all']['frac_under_threshold']:.0%} 가 2 Å 기준
